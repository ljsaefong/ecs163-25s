//add zoom + panning for second interaction
export function drawScatterPlot(data, colorScale, animated = false) {
...
//detect keys
  ...
  //tracker for brushing
  const circles = g.selectAll("circle")
    .data(points, d => d["Timestamp"] || Math.random());
  ...
  circles.join(
    ...
    update => update.transition().duration(animated ? 500 : 0)
      .attr("cx", d => x(+d[hoursKey]))
      .attr("cy", d => y(+d[depressionKey])),

    exit => exit.transition().duration(animated ? 500 : 0)
      .attr("r", 0).remove()
  );
